...
## Supported versions
Receiptly is pre-1.0. Only the latest `main` and most recent tagged release receive security fixes.
...
Receiptly is **on-device and offline by design**:
- No backend, no API keys, no authentication — there is no server to attack.
- Receipt images and the expense database stay in app-private storage and are **never uploaded**.
